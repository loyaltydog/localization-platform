...
export function loadPushNotification(locale, key, titleParams = {}, bodyParams = {}) {
  ...
  try {
    ...
    // Handle string value (legacy SMS templates)
    if (typeof value === 'string') {
      const interpolated = interpolateParams(value, { ...titleParams, ...bodyParams });
      return { title: interpolated, body: interpolated };
    }
 ...
 * ```
 */
export function getPushNotification(memberLanguage, merchantLanguage, key, params, fifthArg) {
  const locale = resolveLocale(memberLanguage, merchantLanguage);
